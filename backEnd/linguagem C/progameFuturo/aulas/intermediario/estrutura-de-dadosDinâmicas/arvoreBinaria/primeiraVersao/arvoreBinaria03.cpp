...
/*
		Aula 268: Como Buscar em uma Árvore Binária de Busca?
			VERSÃO RECURSIVA.
...
// toda vez que tem retorno -> deixa mais lenta.
...
// terceira versã0 -> Forma iterativa, sem recursão

void inserirTerceiraVersao(t_NoArv **raiz, int dado) {
	// como n temos recursividade, precisamos de um loop
	t_NoArv *aux = *raiz; // variavel auxiliar para n alterar o valor da raiz
	while(aux) { // enquanto aux for diferente de NULL;
		
		if(dado < (*raiz)->valor)
			// nao usa *raiz, porque eu quero alterar o conteudo da variavel raiz (localmente, ou seja, sem *)
			raiz = &aux->esquerda; 
		else 
			raiz = &aux->direita;
		
		// raiz vai receber o endereço de memoria do nó, dependendo se o dado é < ou > que o dado que está no nó
		
		// nesse sentido, aux receberá esse dado, e fazerá todo o procedimento novamente.
		aux = *raiz; // autalizando a variavel aux.
	}
	
	// aux é nulo, ou seja, a primeira inserção
	// alocar memória
	aux = (t_NoArv*)malloc(sizeof(t_NoArv));
	aux->valor = dado; // colocando o dado no aux->valor
	aux->esquerda = NULL; // Esquerda é NULL, visto que é o ultimo nó 
	aux->direita = NULL; // Direita é NULL, visto que é o ultimo nó 
	*raiz = aux; // o Conteúdo da raiz receberá o nó aux.
	
}

// função que retorna um ponteiro
// NULL -> NAO ACHOU ; O NÓ -> Achou
t_NoArv *buscarPrimeiraVersao(t_NoArv *raiz, int dado) { // dado 50
	
	// caso o nó não exista
	if(!raiz) // retorna null
		return NULL;
		
	// caso o valor do nó seja igual ao dado
	if(raiz->valor == dado) 
		return raiz; // retorna o nó
		
	// ta dando erro.
	// caso o dado < raiz->valor
	if(dado < raiz->valor) // ir para esquerda
		raiz = buscarPrimeiraVersao(raiz->esquerda, dado);
	
	
	// caso o dado > raiz->valor;
	if(dado > raiz->valor) // ir para direita
		raiz = buscarPrimeiraVersao(raiz->direita, dado);
}
	...
	do {
		...
		switch(opcao) {
			...
			case 1 : 
				...
				inserirTerceiraVersao(&raiz, dado);
				break;
				
			case 2 :
				printf("\nDigite o dado: ");
				scanf("%d", &dado);
				buscarDado = buscarPrimeiraVersao(raiz, dado);
				if(buscarDado) 
					printf("\nDado [%d] foi encontrado...\n", buscarDado->valor);
				else
					printf("\nDado [%d] não foi encotrado...\n", dado);
				break;
