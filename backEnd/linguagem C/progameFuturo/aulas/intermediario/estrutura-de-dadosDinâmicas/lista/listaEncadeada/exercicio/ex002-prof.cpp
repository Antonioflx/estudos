...
// procedimento para organizar lista
// recebendo como parametros os ponteiros das listas.
void dividir(t_No **lista, t_No **listaPar, t_No **listaImpar) {
	t_No *aux = *lista; // criando var auxiliar
	
	// loop para pegar os nós da lista.
	while(aux) {
		if(aux->valor % 2 == 0) // se for par
			inserirInicio(listaPar, aux->valor);
		else // impar
			inserirInicio(listaImpar, aux->valor);
...
void menu(t_No *lista) {
	...
	do {
		...
		switch(opcao) {
			...
			case 8 :
				dividir(&lista, &listaPar, &listaImpar);
				break;
			default:
				printf("Saindo...");
				break;
