...
// procedimento para unir as listas
// origem -> lista que contém os dados que quer ser copiado
// destino -> lista que receberá esses dados.
void unirLista(t_No **listaOrigem, t_No **listaDestino) {
	t_No *aux = *listaOrigem;
	
	while(aux) {
		inserirInicio(listaDestino, aux->valor);
		aux = aux->proximo;
	}
...
void menu(t_No *lista) {
	...
	int opcao, dado, ant;
	t_No *remove, *listaA, *listaB, *listaC;
	...
	do {
		...
		switch(opcao) {
			...
			case 1 :
				printf("\nDigite um dado para por na - listaA: ");
				scanf("%d", &dado);
				inserirInicio(&listaA, dado);
				
				printf("\nDigite um dado para por na - listaB: ");
				scanf("%d", &dado);
				inserirInicio(&listaB, dado);
				break;
			...
			case 2 :	
				...
				scanf("%d", &ant);	
				inserirMeio(&listaA, dado, ant);
				
				printf("\nDigite um dado para por na - listaB: ");
				scanf("%d", &dado);	
				printf("\nQual o dado anterior?  ");
				scanf("%d", &ant);
				inserirMeio(&listaB, dado, ant);
				break;
				
			case 3 :
				printf("\nDigite um dado para por na - listaA: ");
				scanf("%d", &dado);
				inserirFim(&listaA, dado);
				
				printf("\nDigite um dado para por na - listaB: ");
				scanf("%d", &dado);
				inserirFim(&listaB, dado);
				break;
				
			case 4 :
				printf("\nDigite um dado para por na - listaA: ");
				scanf("%d", &dado);
				inserirOrdenado(&listaA, dado);
				
				printf("\nDigite um dado para por na - listaB: ");
				scanf("%d", &dado);
				inserirOrdenado(&listaB, dado);
				
				break;
			case 5 :
				printf("\nDigite um dado para ser removido: ");
				scanf("%d", &dado);
				remove = remover(&listaC, dado);
				if(remove) {
			...
			case 8 :
				unirLista(&listaA, &listaC);
				unirLista(&listaB, &listaC);
				break;
			default:
				printf("Saindo...");
				break;
